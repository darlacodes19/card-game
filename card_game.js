...
// 1. DONE- use a function declaration to create a buildDeck function. 
// 2. DONE- inside the buildDeck function, create an array called "suits" that lists all four suits from a deck of card as strings.
// 3. DONE -inside the buildDeck function, create a 2nd array called "ranks" that lists all 13 cards from ace to King as strings.
// 4. DONE -inside the buildDeck function, create an empty array called "deck"
// 5. DONE- inside the buildDeck function, create a for loop INSIDE of another for loop. The outer loop should loop through the ranks. The inner loop should loop through the suits. Make sure to use different variables for your iterators.
...
// HINT: The result of step 6 is that each card will be an object inside of the deck array, for example [{suit: "diamonds", rank: "A", value: 0}, {suit: "diamonds", rank: "2", value: 1},...{etc}]. For example, if we wanted to organize the players 
          //and teams of the NBA with index numbers, we could write: nba.push({player: players[i], team: teams[n], index: i})
...
//double for loop - took away {} after first for loop to work 

  for (i=0; i<suits.length; i++) 
    for (x=0; x<ranks.length; x++) {   
      let value = x
      deckObject =  {suit: suits[i], rank: ranks[x], value:x}
...
// 1. DONE use a function declaration to create a function called shuffle that takes deck as an argument.
// 2. DONE Inside this function create a variable called "shuffledDeck" that takes deck as its value.
// 3. DONE Using "let" declare three new variables: currentIndex, whos value should equal the length of the deck array, and two more: temporaryValue and randomIndex, each of which should currently have no value assigned.
// 4. DONE Create a while loop whos condition is that "currentIndex" does not equal 0, so that we stop looping once we've gone through all 52 cards.
// 5. DONE Inside the while loop, use the javascript Math.methods to generate a random integer between 0 and "currentIndex"
// 6. DONE Inside the while loop, decrement current index by 1. (should be after step 9)
// 7. DONE Inside the while loop, assign "temporaryValue" with "shuffledDeck" (which is an array) to the [currentIndex].
// 8. DONE Still inside, assign "shuffledDeck[currentIndex]" a value of shuffledDeck to the [randomIndex]
// 9. DONE Still inside, assign "shuffledDeck[randomIndex]" a value of "temporaryValue".  (currentIndex //i--;)
...
const shuffle =  (deck) => {
  ...
  let temporaryValue;
    ...
    while (currentIndex !== 0) { 
      randomIndex =  Math.ceil(Math.random() * (currentIndex - 0) + 0); 
...
const guess = (cardA, cardB) => {
    console.log (cardA.suit, cardA.rank); // ???
    const input = getInput('Do you think the next card will be higher or lower than your current card?');
    ...
    if (input === 'h') {
     ...
     {
      console.log(' please guess higher or lower. No points for this round')
      return false;
    }
...
const playGame =  () => {
  ...
  while (score < 5 && score < deck.length) {
    ...
    if (guess(currentCard, nextCard)) {
      score++
      console.log(`congratulations, you score is now ${score}`)
    }  else {
    console.log('wrong answer. You get no points');
    }

    currentCard = nextCard
  }
  
       deck.length === 0 ? console.log('you are out of cards, you lost')  : console.log( 'You won!')
